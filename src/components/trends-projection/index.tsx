...
async function generateBalanceHistory(args: BalanceOnDateArgs) {
	...
	let relevantTransactions = (account.id && cutoff && l) ?
	...
	return nullDates.map(date => ({
		...
		.concat(
			relevantRange.map((date, i) => ({
				date: format(date, 'M/d/YYY'),
				[account.name]: balanceAsOfDate(account, relevantTransactions, date.toISOString()),
			})),
		);
}

function balanceAsOfDate(account: Account, transactions: Transaction[], date: string) {
...
function TrendsProjection() {
	...
	async function update() {
		...
		// const rts = await getProfileDocsInRange<RecurringTransaction>(fromDate, toDate, profile.id, ProfileCollection.RecurringTransactions);
